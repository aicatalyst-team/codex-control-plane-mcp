...
Admin helper печатает готовый MCP client config, ставит hooks и запускает
protocol smoke:
...
- Не выставляйте этот MCP server в сеть без authentication.
...
После PyPI release целевой install path:
...
`mcp`, `model-context-protocol`, `openai-codex`, `codex-desktop`,
`agent-tools`, `python`, `openclaw`.
