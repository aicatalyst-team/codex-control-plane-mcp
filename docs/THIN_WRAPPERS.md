...
This guide started from PR #9 by `nightcityblade` and has been updated for the
current roadmap feature set.
...
| Active turn steering | Adding context often means starting another turn. | `steer_turn` sends extra text to the active turn without creating a second turn. |
| Forked investigation | Alternative analysis can overwrite the original thread context. | `fork_thread` creates a durable branch before optional follow-up work starts. |
...
| Structured output | Plain text reports are hard for automation to parse. | `output_schema` stores readable text and parsed `finalReport.structured` when Codex returns valid JSON. |
...
| Runtime awareness | The caller has to guess models, hooks, skills, and account state. | Runtime capabilities report safe inventory, sandbox readiness, hooks, skills, and redacted account status. |
...
3. You need Plan Mode or code review to be machine-drivable.
4. You need to steer, fork, archive, compact, or inspect an existing thread.
5. You need structured final reports or image inputs.
6. You need to surface approvals or questions without blocking the original call.
7. You need restart recovery, diagnostics, runtime inventory, or a pollable final report.
...
4. Move plan-heavy workflows to `codex_start_plan_workflow` and `codex_approve_plan`.
5. Add review workflows, steering, fork, and lifecycle tools where they remove manual steps.
6. Add startup health checks with `codex_health_summary` and runtime inventory checks with `codex_get_runtime_capabilities`.
